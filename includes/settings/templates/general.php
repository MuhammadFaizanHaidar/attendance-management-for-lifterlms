...
$global_attendance = !empty( $llmsat_options['llmsat_global_attendance']) ? $llmsat_options['llmsat_global_attendance'] : 'no';
	...
	<form action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" method="POST">
		...
		<table class="form-table">
			<tbody>
				<tr valign="top">
					<th scope="row">
						<label for="llmsat_global_attendance">
							<?php _e( 'Allow Attendance Globally', LLMS_At_TEXT_DOMAIN ); ?>
						</label>
					</th>
					<td>
						<input type="checkbox" name="llmsat_global_attendance" id="llmsat_global_attendance"<?php if( $global_attendance == 'on' ) { ?>checked="checked"<?php } ?> />
						<p class="description"><?php _e( 'If enabled it will allow attendance on all the courses', LLMS_At_TEXT_DOMAIN); ?></p>
					</td>
				</tr>
				<tr valign="top">
					<th scope="row">
